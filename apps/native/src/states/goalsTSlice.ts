...
export type GoalsSlice = {
  ...
  selectedGoal?: GoalT;
  completeGoalDay(goalDay: GoalDayT): Promise<void>;
  ...
  viewGoalDay?: GoalDayT;
  onViewDayOpen(goalDay: GoalDayT): void;
  onViewDayClose(): void;
  updateGoalDayNote(goalDay: GoalDayT, note: string): Promise<void>;
...
export const goalsSlice: AppState<GoalsSlice> = (set, get) => ({
  ...
  async persist() {
    ...
    await storage.set('goalsSlice', { goals: goals.map(g => g.toJSON()), coins, selectedGoalId });
  },
  ...
  ...compute(get, state => {
    ...
    if (selectedGoal) {
      const todayDay = selectedGoal.days.find(day => date.isToday(date.parseStartOfDayISO(day.date)));
  ...
  async createGoal(params) {
    const { persist } = get();
    const goal = GoalT.create(params);
    set(state => ({
  ...
  async completeGoalDay(goalDay) {
    ...
    const newGoals = goals.map(g => g.clone());
    const goalIndex = newGoals.findIndex(goal => goal.id === selectedGoal.id);
    newGoals[goalIndex].completeGoalDay(selectedDay.date, false, note);
    fireConfetti();
  ...
  async completeTodayGoalWithCoins() {
    ...
    const newGoals = goals.map(g => g.clone());
    const goalIndex = newGoals.findIndex(goal => goal.id === selectedGoal.id);
    newGoals[goalIndex].completeGoalDay(today, true, note);
    fireConfetti();
  ...
  async updateGoalDayNote(goalDay, note) {
    ...
    const newGoals = goals.map(g => g.clone());
    const goalIndex = newGoals.findIndex(goal => goal.id === selectedGoal.id);
    newGoals[goalIndex].updateGoalDayNote(goalDay.date, note);
